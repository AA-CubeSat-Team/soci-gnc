/*
 * /Users/devantormey/Documents/MATLAB/UW/AACT/soci-gnc/Test/Autocoding_tests/slprj/grt/_sharedutils/eml_dlahqr_HClTWgbm.c
 *
    ...
    while ((!exitg2) && (its < 301)) {
      ...
      if (k + 1 >= i) {
        ...
        for (tst_tmp = m; tst_tmp <= i; tst_tmp++) {
          ...
          if (tst_tmp > m) {
            ba_tmp = (((tst_tmp - 2) << 2) + tst_tmp) - 1;
            for (hoffset = 0; hoffset < nr; hoffset++) {
              v[hoffset] = h[hoffset + ba_tmp];
            }
          }
          ...
          if (nr == 3) {
            ...
            for (nr = tst_tmp - 1; nr + 1 < 5; nr++) {
              hoffset = nr << 2;
              ba_tmp = hoffset + tst_tmp;
              ba_tmp_tmp_0 = ba_tmp - 1;
              ba_tmp_tmp = ba_tmp + 1;
              ba = (h[ba_tmp_tmp_0] + h[ba_tmp] * htmp1) + h[ba_tmp_tmp] * aa;
              hoffset += tst_tmp;
              h[hoffset - 1] = h[ba_tmp_tmp_0] - ba * tst;
              h[hoffset] = h[ba_tmp] - ba * ab;
              h[hoffset + 1] = h[ba_tmp_tmp] - ba * h12;
            }
            ...
            for (hoffset = 0; hoffset < nr; hoffset++) {
              ba_tmp = ((tst_tmp - 1) << 2) + hoffset;
              ba_tmp_tmp_0 = (tst_tmp << 2) + hoffset;
              ba_tmp_tmp = ((tst_tmp + 1) << 2) + hoffset;
              ba = (h[ba_tmp] + h[ba_tmp_tmp_0] * htmp1) + h[ba_tmp_tmp] * aa;
              h[hoffset + ((tst_tmp - 1) << 2)] = h[ba_tmp] - ba * tst;
              h[hoffset + (tst_tmp << 2)] = h[ba_tmp_tmp_0] - ba * ab;
              h[hoffset + ((tst_tmp + 1) << 2)] = h[ba_tmp_tmp] - ba * h12;
            }

            for (nr = 0; nr < 4; nr++) {
              ba_tmp = ((tst_tmp - 1) << 2) + nr;
              ba_tmp_tmp_0 = (tst_tmp << 2) + nr;
              hoffset = (tst_tmp + 1) << 2;
              ba_tmp_tmp = hoffset + nr;
              ba = (z[ba_tmp] + z[ba_tmp_tmp_0] * htmp1) + z[ba_tmp_tmp] * aa;
              z[nr + ((tst_tmp - 1) << 2)] = z[ba_tmp] - ba * tst;
              z[nr + (tst_tmp << 2)] = z[ba_tmp_tmp_0] - ba * ab;
              z[nr + hoffset] = z[ba_tmp_tmp] - ba * h12;
            }
          } else {
            if (nr == 2) {
              for (nr = tst_tmp - 1; nr + 1 < 5; nr++) {
                ...
                ba_tmp = hoffset + tst_tmp;
                ba_tmp_tmp_0 = ba_tmp - 1;
                ba = h[ba_tmp_tmp_0] + h[ba_tmp] * htmp1;
                hoffset += tst_tmp;
                h[hoffset - 1] = h[ba_tmp_tmp_0] - ba * tst;
                h[hoffset] = h[ba_tmp] - ba * ab;
              }
              ...
              for (nr = 0; nr <= i; nr++) {
                ba = h[((tst_tmp - 1) << 2) + nr] + h[(tst_tmp << 2) + nr] *
                  htmp1;
                h[nr + ((tst_tmp - 1) << 2)] -= ba * tst;
                h[nr + (tst_tmp << 2)] -= ba * ab;
              }

              for (nr = 0; nr < 4; nr++) {
                hoffset = (tst_tmp - 1) << 2;
                ba_tmp_tmp_0 = hoffset + nr;
                ba_tmp_tmp = tst_tmp << 2;
                ba_tmp = ba_tmp_tmp + nr;
                ba = z[ba_tmp_tmp_0] + z[ba_tmp] * htmp1;
                z[nr + hoffset] = z[ba_tmp_tmp_0] - ba * tst;
                z[nr + ba_tmp_tmp] = z[ba_tmp] - ba * ab;
              }
            }
          }
        }
