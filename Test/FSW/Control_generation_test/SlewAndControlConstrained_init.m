...
reorientation.torque_max = .7*3.2*10^-3; %N-m 70% max for any given of our reaction wheel 
m = reorientation.torque_max;
reorientation.T = diag([1/m, 1/m, 1/m, 1/m]);

reorientation.zeta = sqrt(2)/2;   % damping ratio
reorientation.wn = .5;            % natural frequency
...
reorientation.ep = .001; % use this to determine if K needs to be changed
