...
% Initial Conditions %%%%%
    ...
    estimation.ic.quat_est_init = [1;0;0;0]; % Initialize quaternion initial guess
    estimation.ic.activate_triad = 1; % activate Triad on first appropriate timestep (TriadActiavtion_lib)
...
% Sensor Standard Deviations
    sun_sensor_std = 0.5/(sqrt(3)*3.0)*pi/180; % sun sensor measurement std deviation (radians)
    mag_sens_std  = ([4.0305e-07;2.4100e-07;1.7321e-07]);% magnetometer std deviation (micro tesla)
...
% Process and measurement covariances
    ...
    Q_k = [(sig_v^2*dt + 1/3*sig_u^2*dt^3)*eye(3)   -(1/2*sig_u^2*dt^2)*eye(3); %create dynamic noise measurement matrix
                       -(1/2*sig_u^2*dt^2)*eye(3)              (sig_u^2*dt)*eye(3)]; 
     
% Constant Matrices sqrt form
    gamma = blkdiag(-eye(3),eye(3));
    Q = gamma*Q_k*gamma'; %Time Propagation Dynamic Noise Matrix
